...
import AddTelecom from "./AddTelecom";
import {
  Plus,
  Minus,
  Search,
...
const HeaderDetail = ({
  ...
  useEffect(() => {
    const fetchRepository = async () => {
      try {
        ...
        console.error("Lỗi kết nối dữ liệu kho điện tử viễn thông!", error);
        toast.error("Lỗi khi kết nối dữ liệu kho điện tử viễn thông!");
      }
    };
  ...
  useEffect(() => {
    const historyKey = "telecom_search_history";
  ...
  const handleExport = async () => {
    try {
      const res = await axiosClient.get("/export/telecom/export-excel", {
        responseType: "blob",
      });
      ...
      link.href = url;
      link.setAttribute("download", "kho-dienTuVienThong.xlsx");
