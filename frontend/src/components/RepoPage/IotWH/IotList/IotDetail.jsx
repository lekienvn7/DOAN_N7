...
import IotEdit from "@/components/RepoPage/IotWH/IotEdit";
import cadivi from "@/assets/images/cadivi225.png";

const IotDetail = ({ item, open, onClose, onReload }) => {
  if (!item) return null;
        ...
        <div className="flex gap-[48px] p-[40px]">
          ...
          <div className="flex flex-col flex-1">
            ...
            <div className="max-h-[260px] overflow-y-auto pr-4 space-y-3">
              ...
              <Spec label="Công nghệ không dây" value={item.wirelessTech} />
              <Spec label="Xung nhịp CPU" value={item.cpuClock} />
              <Spec label="Bộ nhớ" value={item.memory} />
              <Spec label="Nguồn cấp" value={item.powerSupply} />
              <Spec label="Số chân I/O" value={item.ioPins} />
              <Spec label="Chuẩn giao tiếp" value={item.interfaceType} />
              <Spec label="Điện áp hoạt động" value={item.operatingVoltage} />
              <Spec label="Dòng tiêu thụ" value={item.currentConsumption} />
              <Spec label="Nhiệt độ hoạt động" value={item.operatingTemp} />
              <Spec label="Chuẩn bảo vệ" value={item.protectionLevel} />
              <Spec label="Kích thước" value={item.dimensions} />
...
const Spec = ({ label, value }) => (
  <div className="flex justify-between gap-6 text-sm">
    <span className="text-[#5eead4]">{label}</span>
