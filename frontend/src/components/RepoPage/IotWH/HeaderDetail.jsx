...
const HeaderDetail = ({
  ...
  const checkPermission = () => {
    const hasAccess =
      user?.yourRepo?.includes("all") || user?.yourRepo?.includes("iot");
  ...
  useEffect(() => {
    const fetchRepository = async () => {
      try {
        ...
        console.error("Lỗi kết nối dữ liệu kho nhúng và iot!", error);
        toast.error("Lỗi khi kết nối dữ liệu kho nhúng và iot!");
      }
    };
  ...
  useEffect(() => {
    const historyKey = "iot_search_history";
  ...
  const handleExport = async () => {
    try {
      const res = await axiosClient.get("/export/iot/export-excel", {
        responseType: "blob",
      });
      ...
      link.href = url;
      link.setAttribute("download", "kho-nhung.xlsx");
    ...
    <div className=" flex flex-col p-[20px] w-[100vw] h-[150px] bg-bgmain border-t-1 border-gray-700">
      <div className="flex flex-col gap-[5px]">
        <AnimatePresence>
          ...
          >
            <div className="flex flex-col gap-[5px]">
              <p className="text-left text-[16px] text-[#5eead4] font-satoshi ">
                Kho nhúng và iot
              </p>
            ...
            <Dialog>
              ...
              <DialogContent className="bg-[#1a1a1a] rounded-[12px] border-none text-white ">
                <DialogHeader>
                  <DialogTitle>
                    Thông tin chi tiết{" "}
                    <span className="text-[#5eead4]">kho nhúng và iot</span>
                  </DialogTitle>
                </DialogHeader>
        ...
        <div className="flex flex-row mt-[20px] justify-between">
          <div className="flex text-textsec whitespace-nowrap text-sm cursor-pointer">
            <Dialog open={open} onOpenChange={setOpen}>
              <DialogTrigger asChild>
                <button
                  ...
                  className="pr-3 cursor-pointer flex flex-row gap-[10px] hover:text-[#5eead4] transition-colors duration-300"
                >
              ...
              <DialogContent className="bg-[#1a1a1a] !max-w-none rounded-[12px] p-[25px] w-fit border-none text-white">
                <DialogHeader>
                  ...
                  <DialogDescription className="text-gray-400">
                    ...
                    <span className="text-[#5eead4]">tô màu</span> không thể để
                    trống!
                  </DialogDescription>
                </DialogHeader>
                <AddIot onReload={onReload} />
              </DialogContent>
            </Dialog>
            ...
            <button
              ...
              className="pl-3 cursor-pointer flex flex-row gap-[10px] hover:text-[#5eead4] transition-colors duration-300"
            >
          ...
          <div className="flex flex-row relative">
            <div className=" flex flex-row">
              ...
              >
                {showSearch ? (
                  <ChevronsRight
                    className={`searchClose cursor-pointer hover:text-[#87f3e1] mr-[15px] transition-colors duration-300 text-[#5eead4] no-outline`}
                    size={18}
                  />
                ) : (
                  <Search
                    className={`searchTool cursor-pointer hover:text-[#5eead4] mr-[15px] transition-colors duration-300 text-[#A1A1A6] no-outline`}
                    size={18}
                  />
                )}
              </button>
            ...
            <div className="flex flex-row gap-[10px] p-[0px_15px]">
              ...
              >
                <ArrowDownUp
                  className={`${
                    sortMode ? "text-[#5eead4] " : "text-[#A1A1A6] "
                  } cursor-pointer hover:text-[#87f3e1] transition-colors duration-300`}
                  size={18}
                />
              </button>
              ...
              >
                <Download
                  className="text-[#A1A1A6] cursor-pointer hover:text-[#5eead4] transition-colors duration-300"
                  size={18}
                />
              </button>
            ...
            <button
              ...
              className="text-[14px] ml-[5px] flex flex-row gap-[10px] hover:text-[#5eead4] transition-colors duration-300 cursor-pointer ml-[15px]"
            >
