...
const AutomotiveCard = ({ item }) => {
  const [open, setOpen] = useState(false);

  return (
    <>
      <div onClick={() => setOpen(true)} className="group relative flex flex-col justify-between min-w-[450px] min-h-[500px] bg-[var(--bg-panel)] rounded-[24px] p-8 cursor-pointer hover:scale-[1.02] transition">
        <h3 className="text-[20px] font-semibold">{item.name}</h3>

        <img src={cadivi} alt={item.name} className="w-full h-[250px] object-contain group-hover:scale-[1.03] transition" />

        <div className="flex justify-between items-center">
          <p className="text-[13px] text-[var(--text-tertiary)] line-clamp-2">
            {item.description || "Linh kiện công nghệ ô tô"}
          </p>
      ...
      <AutomotiveDetail open={open} item={item} onClose={() => setOpen(false)} />
    </>
  );
};
