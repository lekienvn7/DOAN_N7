...
import AutomotiveEdit from "@/components/RepoPage/AutomotiveWH/AutomotiveEdit";
import cadivi from "@/assets/images/cadivi225.png";
const AutomotiveDetail = ({ item, open, onClose, onReload }) => {
  if (!item) return null;
        ...
        <div className="flex gap-[48px] p-[40px]">
          ...
          <div className="flex flex-col flex-1">
            ...
            <div className="max-h-[260px] overflow-y-auto pr-4 space-y-3">
              ...
              <Spec label="Loại linh kiện" value={item.partType} />
              <Spec label="Chất liệu" value={item.material} />
              <Spec label="Tiêu chuẩn chất lỏng" value={item.fluidSpec} />
              <Spec label="Tuổi thọ linh kiện" value={item.lifespan} />
              <Spec label="Dòng xe tương thích" value={item.compatibility} />
              <Spec label="Mức chịu nhiệt" value={item.heatResistance} />
              <Spec label="Điện áp hoạt động" value={item.operatingVoltage} />
              <Spec label="Áp suất làm việc" value={item.pressureRating} />
...
const Spec = ({ label, value }) => (
  <div className="flex justify-between gap-6 text-sm">
    <span className="text-[#fb923c]">{label}</span>
