...
const AddAutomotive = ({ onReload }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await axiosClient.put("/repository/automotive", {
        materials: [
          { material: newMaterialID, quantity: Number(basic.quantity) },
        ],
      });

      toast.success("Thêm vật tư công nghệ ô tô thành công!");
  ...
  return (
    <div className="flex flex-col gap-[50px]">
      ...
      <div className="flex flex-col gap-[40px]">
        ...
        {/* ===== AUTOMOTIVE SPECS ===== */}
        <Section title="Thông số ô tô">
          <Row>
            <SelectBox
              label="Loại linh kiện"
              value={spec.partType}
              onChange={(v) => handleSpecChange("partType", v)}
              options={[
                { value: "Engine Components", label: "Nhóm động cơ" },
                { value: "Electronic & Electrical", label: "Nhóm điện" },
                { value: "HVAC", label: "Nhóm điều hòa" },
                { value: "Fluids", label: "Nhóm chất lỏng" },
                { value: "Body & Exterior", label: "Nhóm thân vỏ" },
                { value: "Fasteners", label: "Nhóm ốc vít" },
              ]}
            />
            <Input
              label="Tuổi thọ linh kiện"
              value={spec.lifespan}
              onChange={(v) => handleSpecChange("lifespan", v)}
            />
            <SelectBox
              label="Khả năng chịu nhiệt"
              value={spec.heatResistance}
              onChange={(v) =>
                handleSpecChange("heatResistance", v)
              }
              options={[
                { value: "Low", label: "Thấp" },
                { value: "Medium", label: "Trung bình" },
                { value: "High", label: "Cao" },
                { value: "Extreme", label: "Rất cao" },
                { value: "Ultra", label: "Siêu cao" },
              ]}
            />
          </Row>
          ...
          <Row>
            <Input
              ...
              onChange={(v) =>
                handleSpecChange("vehicleModel", v)
              }
            />
            <Input
              label="Nhà sản xuất"
              ...
              onChange={(v) =>
                handleSpecChange("manufacturer", v)
              }
            />
            <Input
              ...
              onChange={(v) =>
                handleSpecChange("compatibility", v)
              }
            />
          </Row>
          ...
          <Row>
            <Input
              label="Chất liệu"
              value={spec.material}
              onChange={(v) => handleSpecChange("material", v)}
            />
            <Input
              label="Tiêu chuẩn chất lỏng"
              value={spec.fluidSpec}
              onChange={(v) =>
                handleSpecChange("fluidSpec", v)
              }
            />
          </Row>
        </Section>
