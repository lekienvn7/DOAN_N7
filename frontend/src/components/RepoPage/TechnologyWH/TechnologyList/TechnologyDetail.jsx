...
import TechnologyEdit from "@/components/RepoPage/TechnologyWH/TechnologyEdit";
import cadivi from "@/assets/images/cadivi225.png";

const TechnologyDetail = ({ item, open, onClose, onReload }) => {
  if (!item) return null;
        ...
        <div className="flex gap-[48px] p-[40px]">
          ...
          <div className="flex flex-col flex-1">
            ...
            <div className="max-h-[260px] overflow-y-auto pr-4 space-y-3">
              ...
              <Spec label="Loại thiết bị" value={item.deviceType} />
              <Spec label="Dung lượng" value={item.capacity} />
              <Spec label="Tốc độ" value={item.speed} />
              <Spec label="Thương hiệu" value={item.brand} />
              <Spec label="Chuẩn kết nối" value={item.connectorType} />
              <Spec label="Mức tiêu thụ điện" value={item.powerConsumption} />
              <Spec label="Giao thức hỗ trợ" value={item.protocol} />
              <Spec label="Giao diện mạng" value={item.networkInterface} />
              <Spec label="Hệ điều hành" value={item.operatingSystem} />
              <Spec label="Độ bền" value={item.durabilityRating} />
              <Spec label="Chuẩn kích thước" value={item.formFactor} />
              <Spec label="Tốc độ truyền dữ liệu" value={item.dataTransferRate} />
...
const Spec = ({ label, value }) => (
  <div className="flex justify-between gap-6 text-sm">
    <span className="text-[#60a5fa]">{label}</span>
