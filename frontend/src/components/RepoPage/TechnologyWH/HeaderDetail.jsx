...
const HeaderDetail = ({
  ...
  useEffect(() => {
    const fetchRepository = async () => {
      try {
        ...
        console.error("Lỗi kết nối dữ liệu kho công nghệ thông tin!", error);
        toast.error("Lỗi khi kết nối dữ liệu kho công nghệ thông tin!");
      }
    };
  ...
  useEffect(() => {
    const historyKey = "technology_search_history";
  ...
  const handleExport = async () => {
    try {
      const res = await axiosClient.get("/export/technology/export-excel", {
        responseType: "blob",
      });
    ...
    <div className=" flex flex-col p-[20px] w-[100vw] h-[150px] bg-bgmain border-t-1 border-gray-700">
      <div className="flex flex-col gap-[5px]">
        <AnimatePresence>
          ...
          >
            <div className="flex flex-col gap-[5px]">
              <p className="text-left text-[16px] text-[#60a5fa] font-satoshi ">
            ...
            <Dialog open={open} onOpenChange={setOpen}>
              <DialogTrigger asChild>
                <button
                  ...
                  className="pr-3 cursor-pointer flex flex-row gap-[10px] hover:text-[#60a5fa] transition-colors duration-300"
                >
              ...
              <DialogContent className="bg-[#1a1a1a] !max-w-none rounded-[12px] p-[25px] w-fit border-none text-white">
                <DialogHeader>
                  <DialogTitle>
                    Phiếu nhập vật tư{" "}
                    <span className="text-[#60a5fa]">kho Thời trang</span>
                  </DialogTitle>
                  <DialogDescription className="text-gray-400">
                    Nhập vật tư vào kho - Những mục được{" "}
                    <span className="text-[#60a5fa]">tô màu</span> không thể để
                    trống!
                  </DialogDescription>
                </DialogHeader>
                <AddTechnology onReload={onReload} />
              </DialogContent>
            </Dialog>
          ...
          <div className="flex flex-row relative">
            <div className=" flex flex-row">
              ...
              >
                {showSearch ? (
                  <ChevronsRight
                    className={`searchClose cursor-pointer hover:text-[#3184f3] mr-[15px] transition-colors duration-300 text-[#60a5fa] no-outline`}
                    size={18}
                  />
                ) : (
                  <Search
                    className={`searchTool cursor-pointer hover:text-[#60a5fa] mr-[15px] transition-colors duration-300 text-[#A1A1A6] no-outline`}
                    size={18}
                  />
                )}
              </button>
            ...
            {showHistory && history.length > 0 && (
              <motion.div
                ...
                className="absolute top-[30px] left-[35px] w-[270px] max-h-[200px] overflow-y-auto scrollbar-thin scrollbar-thumb-[#60a5fa]/50 hover:scrollbar-thumb-[#e8d6ff]/60  bg-[#111111] border border-gray-400 border-t-[#60a5fa]  rounded-lg shadow-xl z-20"
              >
            ...
            <div className="flex flex-row gap-[10px] p-[0px_15px]">
              ...
              >
                <ArrowDownUp
                  className={`${
                    sortMode ? "text-[#60a5fa] " : "text-[#A1A1A6] "
                  } cursor-pointer hover:text-[#3184f3] transition-colors duration-300`}
                  size={18}
                />
              </button>
              ...
              >
                <Download
                  className="text-[#A1A1A6] cursor-pointer hover:text-[#3184f3] transition-colors duration-300"
                  size={18}
                />
              </button>
            ...
            <button
              ...
              className="text-[14px] ml-[5px] flex flex-row gap-[10px] hover:text-[#3184f3] transition-colors duration-300 cursor-pointer ml-[15px]"
            >
