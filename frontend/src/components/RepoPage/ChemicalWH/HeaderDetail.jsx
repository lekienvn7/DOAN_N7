...
const HeaderDetail = ({ mode, setMode, sortMode, setSortMode }) => {
  ...
  useEffect(() => {
    axiosClient.get("/repository/chemical").then((res) => {
      if (res.data?.success) {
          ...
          <div>
            ...
            <div className="flex flex-row justify-between items-center mt-3 w-[1270px]">
              <p className="text-[22px] font-semibold text-[var(--text-primary)]">
                Danh sách vật tư hóa chất phục vụ giảng dạy và thí nghiệm
              </p>

              <Dialog>
                <DialogTrigger asChild>
                  <button className="flex items-center gap-[6px] text-[20px] text-[var(--accent-blue)] hover:opacity-80 transition">
                    Chi tiết kho <ArrowUpRight size={15} />
                  </button>
                </DialogTrigger>
                ...
                <DialogContent className="bg-white text-[var(--text-primary)] rounded-[20px] border border-[var(--border-light)] shadow-[var(--shadow-md)] max-w-[480px]">
                  <DialogHeader>
                    <DialogTitle className="flex items-center gap-2 text-[20px]">
                      <ToolCase size={18} />
                      Thông tin chi tiết{" "}
                      <span className="text-[#fb923c]">kho hóa chất</span>
                    </DialogTitle>
                  </DialogHeader>
          ...
          <div className="flex items-center gap-6 text-[15px] text-[var(--text-tertiary)]">
            ...
            {repository?.manager?.fullName && (
              <button
                ...
                className="font-medium text-[var(--text-secondary)] hover:text-[var(--accent-blue)] transition-colors"
              >
                {repository.manager.fullName}
              </button>
            )}
